    ...
    def __init__(self, im, line_width=None, font_size=None, font='Arial.ttf', pil=False, example='abc'):
        ...
        if self.pil:  # use PIL
            ...
            try:
                ...
                size = font_size or max(round(sum(self.im.size) / 2 * 0.015), 10)
                self.font = ImageFont.truetype(str(font), size)
            except Exception:
                self.font = ImageFont.load_default()
        else:  # use cv2
            self.im = im
        self.lw = line_width or max(round(sum(im.shape) / 2 * 0.001), 2)  # line width
        # Pose
...
@plt_settings()
def plot_results(file='path/to/trainning_data.csv', dir='', segment=False, pose=False, classify=False):
    ...
    ax = ax.ravel()
    files = list(save_dir.glob('trainning_data*.csv'))
