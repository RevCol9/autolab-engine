...
def convert_mask_to_yolo_format(masks,path):
    f = open(path, "w+")
    for mask in masks:
        H,W = mask.shape[0:2]
        # color = np.array([1, 1, 1, 0.6])
        ...
        #gray_img = cv2.cvtColor(mask_image,cv2.COLOR_BGR2GRAY)
        #ret,bin_img = cv2.threshold(gray_img,0,255,cv2.THRESH_BINARY+cv2.THRESH_OTSU)
        cnt,hit = cv2.findContours(mask_image,cv2.RETR_TREE,cv2.CHAIN_APPROX_TC89_KCOS)
...
if __name__ == "__main__":
    ...
    for i in range(10):
        ...
        if i==0:
            ...
            input_box = np.array([[100, 50, 900, 600]])
            input_point, input_label = None, None
        elif i==9:
        ...
        if not batch_box: 
            ...
            convert_mask_to_yolo_format(masks,i)
        else:
