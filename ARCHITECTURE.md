...
| 服务 | 入口 | 默认端口 | 职责 |
|------|------|----------|------|
| 推理 | `run.py` → `app/main.py` | 21010 | YOLO / Locate / SAM3 在线推理 |
| 训练 | `training/run.py` → `training/api.py` | 21111（env 覆盖） | YOLO 闭环训练子进程调度 |
...
│   ├── main.py                 # FastAPI 路由（薄层）
│   ├── registry.py             # 引擎缓存、GPU 锁、load/unload
│   ├── predict_core.py         # 单图/批量推理逻辑
│   ├── runtime_state.py        # health 探测、unload 辅助
│   ├── gpu_lock.py             # 跨进程 GPU 文件锁（训练也 import）
...
│   ├── hparams.py              # API → train_config.yaml（唯一配置读写）
│   ├── closed_loop_train.py    # 子进程入口（瘦脚本）
...
POST /api/predict
  ...
  → registry.ensure_model_engine（进程内缓存）
  → registry.cross_gpu_session（与训练互斥）
  → engines/*.predict
```
...
`app/gpu_lock.py` 在推理 load/predict 与训练 `JobManager.start` 时各持一把同 index 的文件锁（`/var/run/niii-gpu{N}.lock`）。同一 GPU 上训推不能并行。
